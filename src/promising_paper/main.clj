(ns promising-paper.main
  (:gen-class)
  (:use [promising-paper.core])
  (:require [promising-paper.snps :as snp])
  (:require [promising-paper.input :as input])
  (:require [promising-paper.kernels :as kernel])
  (:require [promising-paper.evaluation :as evaluation])
...
(defn validate
  ...
  (let [truth (into #{} (clojure.string/split (slurp monarch_file) #"\s+"))
    ...
    {"total genes" (count ranked_genes)
     ...
     "enrichment score" (evaluation/gsea-enrichment-scor truth ranked_genes)}))
